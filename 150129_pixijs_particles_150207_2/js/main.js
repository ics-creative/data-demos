    ...
    var ParticleSample = (function (_super) {
        ...
        ParticleSample.prototype.enterFrameHandler = function (event) {
            ...
            for (var i = 0; i < this.pathList.length; i++) {
                ...
                gCurve.beginStroke(colorLine).moveTo(curveStartX, curveStartY).curveTo(p1x, p1y, curveEndX, curveEndY).endStroke();
            }
        ...
        ParticleInitializer.generate = function () {
            ...
            for (var i = 0; i < ParticleInitializer.NUM_PARTICLE; i++) {
                ...
                if (Math.random() < 0.7) {
                    ...
                    shape.graphics.drawRect(-size, -size, size * 2, size * 2).endFill();
                }
                else {
                    if (Math.random() < 0.5) {
                        // キリッとした円
                    ...
                    shape.graphics.moveTo(-size / 2, size / 2).lineTo(0, size / 2 - takasa).lineTo(+size / 2, size / 2).closePath().endFill();
                }
                var padding = 4;
