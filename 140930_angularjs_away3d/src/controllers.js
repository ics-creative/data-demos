...
phonecatApp.controller('MyController', function ($scope) {
    $scope.$watch('sliderX', function (newVal, oldVal) {
        if (view)
            view.camera.x = newVal;
        if ($scope.checkBoxLookAt) {
            updateRotation();
        }
    });
    $scope.$watch('sliderY', function (newVal, oldVal) {
        if (view)
            view.camera.y = newVal;
        if ($scope.checkBoxLookAt) {
            updateRotation();
        }
    });
...
function onLoadImage() {
    ...
    // 地面を作成
    var earth = new away.prefabs.PrimitivePlanePrefab(3000, 3000).getNewObject();
    var materialEarth = new away.materials.TriangleMethodMaterial(away.materials.DefaultMaterialManager.getDefaultTexture());
